...
parser.add_argument('--data_train', type=str, default='/home/wr/03_restore/DATA/Train/image')
# parser.add_argument('--label_train', type=str, default='../Dataset/UIE/UIEBD/train/image')
parser.add_argument('--label_train', type=str, default='/home/wr/03_restore/DATA/Train/label')
...
parser.add_argument('--label_test', type=str, default='/home/wr/03_restore/DATA/test_u190/gt')
parser.add_argument('--rgb_range', type=int, default=1, help='maximum value of RGB')
# parser.add_argument('--im_resize', type=bool, default=True)
# parser.add_argument('--res_w', type=int, default=256, help='Size_w of resized HR image')
# parser.add_argument('--res_h', type=int, default=256, help='Size_h of resized HR image')
...
def train():
    ...
    for iteration, batch in enumerate(training_data_loader, 1):
        ...
        hsv = np_to_torch(rgb2hsv(torch_to_np(j_out).transpose(1, 2, 0)))
        cap_prior = hsv[:, :, :, 2] - hsv[:, :, :, 1]
        cap_loss = mse_loss(cap_prior, torch.zeros_like(cap_prior))

        total_loss = 1 * loss_1 + 1 * loss_2 + 0.01 * loss_3 + 1 * cap_loss
        ...
        print("===> Epoch[{}]({}/{}): Loss: {:.4f} || Learning rate: lr={} || Timer: {:.4f} sec.".format(epoch,
                                                                                                         iteration, len(
                training_data_loader), total_loss.item(), optimizer.param_groups[0]['lr'], (t1 - t0)))
